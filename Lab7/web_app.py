...
@app.route('/view')
def view_file():
    filepath = os.path.join(UPLOAD_FOLDER, 'filename.txt')  # thay bằng tên file bạn nhận
    if os.path.exists(filepath):
        with open(filepath, 'r', encoding='utf-8') as f:
            content = f.read()
        return f"<h2>Nội dung file:</h2><pre>{content}</pre>"
    else:
        return "File chưa tồn tại hoặc chưa được nhận."
